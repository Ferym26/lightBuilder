<?
	...
	$serverEmail 	= "tikohid586@brbqx.com";
	...
	$vName	= filter_input(INPUT_POST, 'fieldNameModal',	FILTER_SANITIZE_STRING);
	$vPhone = filter_input(INPUT_POST, 'fieldPhoneModal',	FILTER_SANITIZE_STRING);
	...
	$message 		= "";
	$message 		.= "Заказ обратного звонка.<br />";
	$message 		.= $vName."<br />";
	$message 		.= $vPhone;
	...
	if($isMail){
		...
		$m->To( $serverEmail );
		$m->Subject( "Форма: Заказать звонок [".$vName."]" );
		$m->Body( $message, "html");    
		$m->Priority(2); 		
		if($smtp_server!=""){
